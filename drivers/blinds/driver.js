...
var driver = 'windowcovering';
var capability = 'windowcoverings_state';
...
module.exports = self = {
		...
		capabilities: {
			...
			windowcoverings_state: {
				...
				set: function( device_data, state, callback ) {
						if(state == 'idle'){
							var device = deviceList.get(device_data);
							state = device.variables.state;
							updated = deviceList.update(self, device_data, 'idle', 'windowcovering') //Turn on the device
						...
						if(state == 'up') var onoff = true;
						else if(state == 'down') var onoff = false;
						if(onoff == undefined) return;
		...
		pair: function( socket ) { //Pair sequence
			...
			socket.on('done', function( data, callback ){
				...
				if(driver == "windowcovering"){
					setTimeout(function(){
						Homey.manager('settings').set( "" + tempdata.address + tempdata.group + tempdata.channel + tempdata.unit + 'invert', pairdata.invert );
