...
exports.createDriver = function(driver, capability) {
	self = {
		...
		received: function(device_data){
			...
			var updated = deviceList.update(self, device_data, device_data.state);
			if(updated) flow.trigger(device_data, updated);
			return;
		},
		...
		capabilities: {
			onoff: {
				...
				set: function( device_data, state, callback ) {
					deviceList.update(self, device_data, state);
			...
			dim: {
				...
				set: function( device_data, dim, callback ) {
					...
					if(dim <= 0.0099){ //Turn the device off when the dimvalue = 0
						deviceList.update(self, device_data, false)
						wireless.sendState(device_data, false); //Send state command to device
					}else{ //If the dim value is not 0
						if(dim == 1) dim = 0.99;
						deviceList.update(self, device_data, dim) //Set the dim value
			...
			windowcoverings_state: {
				...
				set: function( device_data, state, callback ) {
					updated = deviceList.update(self, device_data, state) //Turn on the device
					if(updated) flow.trigger(device_data, updated);
